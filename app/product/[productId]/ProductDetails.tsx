...
const ProductDetails: React.FC<ProductDetailsProps> = ({ product }) => {
  ...
  useEffect(() => {
    ...
    if (cartProducts) {
      //checks whether the product has alr in the cart
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
      ...
      <div className="flex flex-col gap-1 text-slate-500 text-sm">
        <h2 className="text-3xl text-pink font-bold">{product.name}</h2>
        <div className="flex items-center gap-2">
          <Rating value={productRating} readOnly></Rating>
          <div>{product.reviews.length} reviews</div>
        </div>
        <Horizontal />
        <div className="text-justify text-maintext">{product.description}</div>
        <Horizontal />
        <div>
        ...
        {isProductInCart ? (
          <>
            <p className="mb-2 text-slate-500 flex items-center gap-1">
              ...
              <span className="text-accent">Prodoct added to cart</span>
            </p>
